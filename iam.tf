resource "oci_identity_dynamic_group" "app_server" {
  compartment_id = var.oci_tenancy_id
  name           = "AppServer"
  description    = "Contains the AppServer only"
  matching_rule  = "instance.id = '${oci_core_instance.app_server.id}'"

  defined_tags = merge(local.default_tags, {
    "terraform.name" = "AppServer"
  })
}
...
resource "oci_identity_policy" "access_backup_bucket" {
  ...
  statements = [
    <<-POLICY
      Allow dynamic-group ${oci_identity_dynamic_group.app_server.name} to manage object-family in compartment id
    ...
    ,
    <<-POLICY
      Allow dynamic-group ${oci_identity_dynamic_group.app_server.name} to use keys in compartment id
      ${data.terraform_remote_state.oci_core.outputs.terraform_identity_compartment_id}
